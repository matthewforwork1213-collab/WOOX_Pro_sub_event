...
> **Feature 1 and Feature 2 have completely independent data sources with no interrelation** (confirmed 2026-07-03). Feature 1 reverse-calculates from **actually occurred historical data** in the user UID Table. (Each withdrawal is identified by a server-issued `paybackNo` (payback-request record ID) — the paybackNo record is created at the withdrawal-request-complete moment and carries exchange · UID · payback, so the API request passes a list of paybackNo and the server resolves the records. Even though **the same UID can exist on different exchanges**, the single paybackNo key identifies it unambiguously.) Feature 2 (Cashback Preview), independent of the UID Table, is a pure projection where — since the user's actual trading volume is unknown — **a predetermined formula computes a predicted value from inputs (assets, leverage, frequency, etc.) alone**. Below, the "Nominal basis" is exclusive to Feature 1, and the "Preview basis" is exclusive to Feature 2; they do not share data with each other.
