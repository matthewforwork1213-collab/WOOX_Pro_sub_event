...
| REQ-005 | Feature 1 | For withdrawals that include WOOX Pro, display in this priority order: (1) WOOX Pro's own event, (2) TetherMax's WOOX Pro Onboarding Event, (3) TetherMax's general House Ad | Must | If WOOX Pro's own event is active in the Admin, the WOOX Pro event is displayed with top priority even if other exchange events exist |
...
| ID | Category | Requirement | Priority | Acceptance Criteria |
|---|---|---|---|---|
| REQ-016 | Feature 3 | During the promotion's active period, display the Travel Rule Integration Banner in 5 locations (bottom of the Cashback Preview results, outside the bottom of the withdrawal result card, below the pre-login button on MO, top of the PC login page, below the member ID on My Page after login) | Must | The banner is correctly displayed in all 5 locations (only S1 and S2 apply to the App) |
| REQ-017 | Feature 3 | The banner is composed, in this order, of `[Bithumb logo]` `[WOOX Pro logo]` (hardcoded in the design) + copy (KO "트레블룰 연동" / EN "Travel Rule Integration"), and this order is maintained across all languages | Must | Even when the language is switched, the logo-copy order is maintained and only the copy is translated |
| REQ-018 | Feature 3 | The banner is a static banner that does not navigate to any screen when clicked | Must | No action occurs when the banner is clicked (tapped) |
| REQ-019 | Feature 3 | Display of the banner on pre-login screens (#3, #4) is determined solely by whether the promotion is globally active, regardless of the user's session | Must | The banner is displayed even in a logged-out state, as long as the promotion is within its active period |

### 1.4 Common Requirements (Promotion Active/Termination, Data, and Display Rules)

| ID | Category | Requirement | Priority | Acceptance Criteria |
|---|---|---|---|---|
| REQ-020 | Common | The promotion's active status is determined as "(within D+30 of the WOOX Pro onboarding point) AND (neither of the 2 WOOX Pro events has ended)" | Must | Upon reaching D+31, or when 1 of the 2 events ends, the promotion immediately switches to inactive |
| REQ-021 | Common | When the promotion switches to inactive, Features 1, 2, and 3 are simultaneously and collectively rolled back to the base logic (no partial termination) | Must | When the termination trigger occurs, all three features revert to the base state at the same point in time |
| REQ-022 | Common | Early termination of the promotion uses the existing end button on the 2 existing WOOX Pro events' Admin lookup screen as-is; no new termination UI is developed | Must | The REQ-021 behavior is triggered simply by clicking the existing end button, with no new Admin screen |
...
| Won't (excluded, out of scope for this project) | Making the promotion permanent (OI-07 closed), fixing the cashback engine's double-multiplication bug (OI-10, separate track), developing a new Admin termination UI | approx. 4% |
